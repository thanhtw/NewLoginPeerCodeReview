...
"已識別的問題": [
    ...
    "行號": 42,
    "程式碼片段": "String str = null; int length = str.length();",
    "說明": "此代碼將引發NullPointerException，因為它在空String上調用length()"
    }
    // 列出所有與請求列表匹配的已實現錯誤
],
"遺漏的問題": [
...
2. 識別學生正確找出的已知問題
3. 注意學生遺漏的已知問題
4. 評估審查質量（準確性、完整性、清晰度和具體性）
5. 確定審查是否足夠（正確識別>=60%的問題）
